...
export const updateUserRole = catchAsync(async (req, res,next) => {
  ...
  if (user.role === 'super_admin' && role !== 'super_admin') {
    ...
    if (superAdminCount === 1) return next(new AppError('لا يمكن تغيير صلاحية السوبر أدمن الوحيد', 400));
  }
  user.role = role;
  await user.save();
  res.json({ success: true, message: 'تم تحديث الصلاحية', user: { id: user._id, role: user.role } });
});

// export const toggleUserStatus = catchAsync(async (req, res,next) => {
//   const user = await User.findById(req.params.id);
//   if (!user) return next(new AppError('مستخدم غير موجود', 404));
//   if (user.role === 'super_admin') return next(new AppError('لا يمكن تعطيل السوبر أدمن', 403));
//   user.isActive = !user.isActive;
//   await user.save();
//   res.json({ success: true, message: `تم ${user.isActive ? 'تفعيل' : 'تعطيل'} الحساب` });
// });
export const deleteUser = catchAsync(async (req, res,next) => {
  ...
  if (user.role === 'super_admin') {
    const superAdminCount = await User.countDocuments({ role: 'super_admin' });
    if (superAdminCount === 1) return next(new AppError('لا يمكن حذف السوبر أدمن الوحيد', 400));
  }
