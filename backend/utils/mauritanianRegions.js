// utils/mauritanianRegions.js

export const mauritanianRegions = {
  // ==================== العربية ====================
  'الحوض الشرقي': {
    'النعمة': ['النعمة'],
    'آمرج': ['آمرج'],
    'عدل بكرو': ['عدل بكرو'],
    'باسكنو': ['باسكنو'],
    'جگني': ['جگني'],
    'تمبدغة': ['تمبدغة'],
    'ولاتة': ['ولاتة'],
    'انبيكت لحواش': ['انبيكت لحواش']
  },
  'الحوض الغربي': {
    'العيون': ['العيون'],
    'الطينطان': ['الطينطان'],
    'كوبني': ['كوبني'],
    'تامشكط': ['تامشكط'],
    'اطويل': ['اطويل']
  },
  'العصابة': {
    'كيفة': ['كيفة'],
    'باركيول': ['باركيول'],
    'بومديد': ['بومديد'],
    'كنكوصة': ['كنكوصة'],
    'گرو': ['گرو']
  },
  'كوركول': {
    'كيهيدي': ['كيهيدي'],
    'مقامه': ['مقامه'],
    'مونكل': ['مونكل'],
    'امبود': ['امبود'],
    'لكصيبة': ['لكصيبة']
  },
  'البراكنة': {
    'ألاك': ['ألاك'],
    'بابابي': ['بابابي'],
    'بوگي': ['بوگي'],
    'مقطع لحجار': ['مقطع لحجار'],
    'مال': ['مال'],
    'امباني': ['امباني']
  },
  'الترارزة': {
    'روصو': ['روصو'],
    'بوتليميت': ['بوتليميت'],
    'اركيز': ['اركيز'],
    'المذرذرة': ['المذرذرة'],
    'كرمسين': ['كرمسين'],
    'واد الناقة': ['واد الناقة'],
    'تيكان': ['تيكان']
  },
  'آدرار': {
    'أطار': ['أطار'],
    'أوجفت': ['أوجفت'],
    'شنقيط': ['شنقيط'],
    'وادان': ['وادان']
  },
  'داخلت نواذيبو': {
    'نواذيبو': ['نواذيبو'],
    'الشامي': ['الشامي']
  },
  'تكانت': {
    'تجكجة': ['تجكجة'],
    'تيشيت': ['تيشيت'],
    'المجرية': ['المجرية']
  },
  'كيدي ماغا': {
    'سيلبابي': ['سيلبابي'],
    'ولد ينج': ['ولد ينج'],
    'غابو': ['غابو'],
    'ومبو': ['ومبو']
  },
  'تيرس زمور': {
    'ازويرات': ['ازويرات'],
    'افديرك': ['افديرك'],
    'بير أم اكرين': ['بير أم اكرين']
  },
  'إنشيري': {
    'أكجوجت': ['أكجوجت'],
    'بنشاب': ['بنشاب']
  },
  'نواكشوط الغربية': {
    'تفرغ زينة': ['تفرغ زينة'],
    'لكصر': ['لكصر'],
    'السبخة': ['السبخة']
  },
  'نواكشوط الشمالية': {
    'دار النعيم': ['دار النعيم'],
    'تيارت': ['تيارت'],
    'توجنين': ['توجنين']
  },
  'نواكشوط الجنوبية': {
    'عرفات': ['عرفات'],
    'الميناء': ['الميناء'],
    'الرياض': ['الرياض']
  },

  // ==================== الفرنسية ====================
  'Hodh El Chargui': {
    'Néma': ['Néma'],
    'Amourj': ['Amourj'],
    'Adel Bagrou': ['Adel Bagrou'],
    'Bassiknou': ['Bassiknou'],
    'Djiguenni': ['Djiguenni'],
    'Timbedgha': ['Timbedgha'],
    'Oualata': ['Oualata'],
    'Nbeiket Lahwach': ['Nbeiket Lahwach']
  },
  'Hodh El Gharbi': {
    'Ayoun el Atrous': ['Ayoun el Atrous'],
    'Tintane': ['Tintane'],
    'Kobenni': ['Kobenni'],
    'Tamchekett': ['Tamchekett'],
    'Touil': ['Touil']
  },
  'Assaba': {
    'Kiffa': ['Kiffa'],
    'Barkéol': ['Barkéol'],
    'Boumdeid': ['Boumdeid'],
    'Kankossa': ['Kankossa'],
    'Guerou': ['Guerou']
  },
  'Gorgol': {
    'Kaédi': ['Kaédi'],
    'M\'Bout': ['M\'Bout'],
    'Monguel': ['Monguel'],
    'Maghama': ['Maghama'],
    'Lexeiba': ['Lexeiba']
  },
  'Brakna': {
    'Aleg': ['Aleg'],
    'Bababé': ['Bababé'],
    'Boghé': ['Boghé'],
    'Magta Lahjar': ['Magta Lahjar'],
    'Mâl': ['Mâl'],
    'M\'Bagne': ['M\'Bagne']
  },
  'Trarza': {
    'Rosso': ['Rosso'],
    'Boutilimit': ['Boutilimit'],
    'Arkiz': ['Arkiz'],
    'El Mederdra': ['El Mederdra'],
    'Keur Macène': ['Keur Macène'],
    'Oued Naga': ['Oued Naga'],
    'Tékane': ['Tékane']
  },
  'Adrar': {
    'Atar': ['Atar'],
    'Aoujeft': ['Aoujeft'],
    'Chinguetti': ['Chinguetti'],
    'Ouadane': ['Ouadane']
  },
  'Dakhlet Nouadhibou': {
    'Nouadhibou': ['Nouadhibou'],
    'Chami': ['Chami']
  },
  'Tagant': {
    'Tidjikja': ['Tidjikja'],
    'Tichit': ['Tichit'],
    'Moudjeria': ['Moudjeria']
  },
  'Guidimaka': {
    'Sélibaby': ['Sélibaby'],
    'Ould Yengé': ['Ould Yengé'],
    'Ghabou': ['Ghabou'],
    'Wompou': ['Wompou']
  },
  'Tiris Zemmour': {
    'Zouérat': ['Zouérat'],
    'F\'dérick': ['F\'dérick'],
    'Bir Moghrein': ['Bir Moghrein']
  },
  'Inchiri': {
    'Akjoujt': ['Akjoujt'],
    'Benichab': ['Benichab']
  },
  'Nouakchott Ouest': {
    'Tevragh-Zeina': ['Tevragh-Zeina'],
    'Ksar': ['Ksar'],
    'Sebkha': ['Sebkha']
  },
  'Nouakchott Nord': {
    'Dar Naim': ['Dar Naim'],
    'Teyarett': ['Teyarett'],
    'Toujounine': ['Toujounine']
  },
  'Nouakchott Sud': {
    'Arafat': ['Arafat'],
    'El Mina': ['El Mina'],
    'Riyad': ['Riyad']
  }
};

// دوال مساعدة
export const getAllWilayas = () => Object.keys(mauritanianRegions);

export const getdairasByWilaya = (wilaya) => {
  return mauritanianRegions[wilaya] ? Object.keys(mauritanianRegions[wilaya]) : [];
};

export const getBaladiasBydaira = (wilaya, daira) => {
  return mauritanianRegions[wilaya]?.[daira] || [];
};

export const validateWilaya = (wilaya) => {
  return mauritanianRegions[wilaya] !== undefined;
};

export const validateDaira = (wilaya, daira) => {
  return mauritanianRegions[wilaya] && mauritanianRegions[wilaya][daira] !== undefined;
};

// البحث في العربية أو الفرنسية
export const searchWilaya = (searchTerm) => {
  const allWilayas = getAllWilayas();
  return allWilayas.filter(wilaya => 
    wilaya.toLowerCase().includes(searchTerm.toLowerCase())
  );
};